...
window.webxdc = (() => {
  ...
  return {
    ...
    sendUpdate: (update, description) => {
      ...
      console.log(
        '[Webxdc] description="' + description + '", ' + JSON.stringify(update)
      );
      updateListener(update);
      var updatesJSON = window.localStorage.getItem(updatesKey);
...
window.alterXdcApp = () => {
  var styleControlPanel =
    "position: fixed; bottom:1em; left:1em; background-color: #000; opacity:0.8; padding:.5em; font-family: sans-serif; color:#fff; z-index: 9999";
  var styleMenuLink =
  ...
  if (window.webxdc.selfName() === "device0") {
    ...
    div.innerHTML =
      ...
      styleMenuLink +
      '"> | </span>' +
      '<a href="javascript:window.clearXdcStorage();" style="' +
      styleMenuLink +
      '">Clear Storage</a>' +
      "<div>";
